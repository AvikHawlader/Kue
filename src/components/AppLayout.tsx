...
import { MessageSquare, Settings, LogOut, Menu, X, HelpCircle, Zap, Clock, ShieldCheck, LifeBuoy } from 'lucide-react';
import { supabase } from '../lib/supabaseClient';

interface AppLayoutProps {
  children: React.ReactNode;
  currentPage: 'chat' | 'settings' | 'help' | 'privacy' | 'terms';
  onNavigate: (page: 'chat' | 'settings' | 'help' | 'privacy' | 'terms') => void;
  credits: number | null;
        ...
        <div className="mt-auto shrink-0">
          ...
          <div className="p-4 border-t border-slate-200 space-y-1 bg-white/50">
            ...
            <div className="flex items-center justify-center gap-3 pt-2 pb-1">
              ...
              <button onClick={() => onNavigate('terms')} className="text-[10px] text-slate-400 hover:text-indigo-500 hover:underline">Terms</button>
            </div>
          </div>
        </div>
      </aside>
      ...
      >
        ...
        <div className="pb-8 space-y-4 mt-auto">
          ...
          <div className="border-t border-slate-100 pt-2 space-y-1">
            ...
            <a 
              href="mailto:avikhawlader2002@gmail.com?subject=Kue%20Support%20Request"
              className="flex items-center space-x-3 text-slate-500 w-full px-4 py-3 hover:text-indigo-600"
            >
              <LifeBuoy size={20} />
              <span>Contact Support</span>
            </a>

             <div className="flex items-center justify-center gap-4 px-4 pt-2 pb-6">
                <button onClick={() => handleNavClick('privacy')} className="text-xs text-slate-400 underline">Privacy</button>
                <button onClick={() => handleNavClick('terms')} className="text-xs text-slate-400 underline">Terms</button>
             </div>
          </div>
        </div>
      </div>
